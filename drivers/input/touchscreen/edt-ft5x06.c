...
static irqreturn_t edt_ft5x06_ts_isr(int irq, void *dev_id)
{
	...
	int error;
//	printk("touchscreen ------%d---\n",irq);
//	printk("touchscreen is- --tsdata->version--is %d---\n",tsdata->version);
...
//		printk("touchscreen -------x=%d------y=%d--\n",x,y);
		...
		if (input_mt_report_slot_state(tsdata->input, MT_TOOL_FINGER,
			...
			touchscreen_report_pos(tsdata->input, &tsdata->prop,
					       y, x, true);// x ,y changed
	}
	printk("touchscreen 2222-----2x=%d------2y=%d--\n",y,x);
					 ...
					 const struct i2c_device_id *id)
{
	...
	printk("touchscreen probe---------probe---------\n");
